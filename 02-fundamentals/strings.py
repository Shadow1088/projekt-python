...
#print(" ".join((first_name, second_name))*3)
# Opakování řetězců
...
#print(f"{first_name.upper()} {second_name.lower()} {5 * 4} {len(message)}")
...
'''
date_cz = "12. 10. 2020"
date_db = "-".join(list(reversed(date_cz.replace(".", "").split())))
print(date_cz, date_db)
...
'''
def str_to_snake_case(input:str="var"):
    # zde by mohl byt check jestli je nazev validni..
    return "_".join(input.lower().split())

#print(str_to_snake_case("Tohle By mel byt Variable 26"))

def str_to_camel_case(input:str="var"):
    # zde by mohl byt check jestli je nazev validni..
    words = input.lower().replace("_", " ").split()
    fixed_words = [word[0].upper() + word[1:] for word in words]
    finished_words = "".join(fixed_words)
    return finished_words[0].lower() + finished_words[1:]

#print(str_to_camel_case("te ST a"))

def translate(input:str="var", fn=str_to_snake_case):
    return fn(input)

text1 = "spRávny PROGRAMATOR nikdy NEVÍ JAK POJMEnováVat Proměnné!!~_"
text2 = "camel_case_tes__A"
print(translate(text2, str_to_camel_case))
